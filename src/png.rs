...
pub struct Png {
    signature: [u8; 8],
    chunk_layout: Vec<Chunk>,
}

impl Png {
    pub fn from_chunks(chunks: Vec<Chunk>) -> Self {
        Self {
            signature: STANDARD_HEADER,
            chunk_layout: chunks,
        }
    }
    pub fn append_chunk(&mut self, chunk: Chunk) {
        self.chunk_layout.push(chunk);
    }
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, &str> {
        match self
            .chunk_layout
            .iter()
            .position(|f| f.chunk_type().to_string() == chunk_type)
        {
            Some(v) => Ok(self.chunk_layout.remove(v)),
            None => Err("No such chunk exists"),
        }
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        if let Some(i) = self
            ...
            .position(|f| f.chunk_type().to_string() == chunk_type)
        {
            return Some(&self.chunk_layout[i]);
        } else {
            return None;
        }
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut result: Vec<u8> = Vec::from(STANDARD_HEADER);

        for i in self.chunk_layout.clone() {
            result.extend_from_slice(i.as_bytes().as_slice())
        }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let mut chunk_vec = Vec::from_iter(value[8..].iter());
        let mut chunk_layout: Vec<Chunk> = Vec::new();

        while chunk_vec.len() > 1 {
            let length_drain: Vec<u8> = chunk_vec.drain(0..4).map(|f| *f).collect();
            ...
            let chunk_type_drain: Vec<u8> = chunk_vec.drain(0..4).map(|f| *f).collect();
            ...
            let chunk_data_drain: Vec<u8> =
                chunk_vec.drain(0..length as usize).map(|f| *f).collect();

            let chunk_data = Vec::from_iter(chunk_data_drain.iter().cloned());

            let crc_drain: Vec<u8> = chunk_vec.drain(0..4).map(|f| *f).collect();
            let crc = u32::from_be_bytes([crc_drain[0], crc_drain[1], crc_drain[2], crc_drain[3]]);

            if length != chunk_data.len() as u32 {
                return Err("Invalid Data or Length");
            }

            let mut check_sum_vec = Vec::from_iter(chunk_type_drain);
            check_sum_vec.extend_from_slice(chunk_data_drain.as_slice());

            if crc != ISO.checksum(check_sum_vec.as_slice()) {
...
mod tests {
    ...
    pub fn testing_chunks() -> Vec<Chunk> {
    ...
    pub fn testing_png() -> Png {
        let chunks = testing_chunks();
        Png::from_chunks(chunks)
    }

    pub fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, Error> {
    ...
    #[test]
    pub fn test_png_from_image_file() {
